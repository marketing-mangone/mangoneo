...
function ScoreForm({ onSave }: { onSave: (data: Omit<ScoreInput, 'competitor'>) => Promise<void> }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-[#f8f9fb] rounded-xl border border-[#e8eaf0] p-4 space-y-3">
      <p className="text-xs font-semibold text-[#374151] uppercase tracking-wide">Registrar puntuación</p>
      {error && <div className="bg-red-50 text-red-600 text-xs rounded-lg px-3 py-2">{error}</div>}
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className="block text-xs font-medium text-[#6b7280] mb-1">Dimensión</label>
          <select value={form.dimension} onChange={e => setForm(f => ({ ...f, dimension: e.target.value as CompetitorDimension }))}
            className="w-full border border-[#e5e7eb] rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#0C2054]/20">
            {DIMENSIONS.map(d => <option key={d.key} value={d.key}>{d.label}</option>)}
          </select>
        </div>
        <div>
          <label className="block text-xs font-medium text-[#6b7280] mb-1">Puntuación (0–10)</label>
          <input value={form.score} onChange={e => setForm(f => ({ ...f, score: e.target.value }))}
            type="number" min="0" max="10" step="0.1" placeholder="ej. 7.5"
            className="w-full border border-[#e5e7eb] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0C2054]/20" />
        </div>
      </div>
      <div>
        <label className="block text-xs font-medium text-[#6b7280] mb-1">Valor real (opcional)</label>
      ...
      <button type="submit" disabled={loading}
        className="w-full bg-[#0C2054] text-white rounded-lg py-2 text-sm font-semibold hover:bg-[#1a3a7a] disabled:opacity-50 transition-colors flex items-center justify-center gap-2">
...
function AdForm({ onSave }: { onSave: (data: Omit<AdObservationInput, 'competitor'>) => Promise<void> }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-[#f8f9fb] rounded-xl border border-[#e8eaf0] p-4 space-y-3">
      <p className="text-xs font-semibold text-[#374151] uppercase tracking-wide">Registrar ad observado</p>
      {error && <div className="bg-red-50 text-red-600 text-xs rounded-lg px-3 py-2">{error}</div>}
      <div className="grid grid-cols-2 gap-2">
        <div>
        ...
        <div>
          <label className="block text-xs font-medium text-[#6b7280] mb-1">Fecha observado</label>
          <input type="date" value={form.observed_date} onChange={e => setForm(f => ({ ...f, observed_date: e.target.value }))}
            className="w-full border border-[#e5e7eb] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0C2054]/20" />
        </div>
      </div>
      <div>
        <label className="block text-xs font-medium text-[#6b7280] mb-1">Título / Headline</label>
        <input value={form.headline} onChange={e => setForm(f => ({ ...f, headline: e.target.value }))}
          placeholder="Titular del anuncio"
          className="w-full border border-[#e5e7eb] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0C2054]/20" />
      </div>
      <div>
        <label className="block text-xs font-medium text-[#6b7280] mb-1">Mensaje principal</label>
        <textarea value={form.message} onChange={e => setForm(f => ({ ...f, message: e.target.value }))}
          placeholder="Copy del anuncio..." rows={2}
          className="w-full border border-[#e5e7eb] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0C2054]/20 resize-none" />
      </div>
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className="block text-xs font-medium text-[#6b7280] mb-1">Call to Action</label>
          <input value={form.cta} onChange={e => setForm(f => ({ ...f, cta: e.target.value }))}
            placeholder="ej. Llama ahora, Agenda gratis..."
            className="w-full border border-[#e5e7eb] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0C2054]/20" />
        </div>
        <div>
          <label className="block text-xs font-medium text-[#6b7280] mb-1">Diferenciador usado</label>
      ...
      <div>
        <label className="block text-xs font-medium text-[#6b7280] mb-1">URL del creativo (opcional)</label>
...
function AdCard({ ad, onDelete }: { ad: ApiAdObservation; onDelete: (id: number) => void }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-[#e8eaf0] p-4 group">
      <div className="flex items-start justify-between gap-2 mb-2">
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
          ...
          <button onClick={() => onDelete(ad.id)}
            className="w-6 h-6 rounded-lg flex items-center justify-center text-[#9ca3af] hover:text-red-500 hover:bg-red-50 transition-all ml-1">
            <Trash2 className="w-3 h-3" />
          </button>
        </div>
      </div>
      {ad.headline && <h4 className="font-semibold text-[#0C2054] text-sm mb-1">{ad.headline}</h4>}
      {ad.message && <p className="text-[#6b7280] text-xs leading-relaxed mb-2">{ad.message}</p>}
      {(ad.cta || ad.differentiator) && (
        <div className="flex flex-wrap gap-2 mt-2">
          {ad.cta && (
            <span className="text-[10px] bg-[#F79C31]/10 text-[#b8720f] px-2 py-1 rounded-lg font-medium">
              CTA: {ad.cta}
            </span>
          )}
          {ad.differentiator && (
            <span className="text-[10px] bg-[#0C2054]/[0.08] text-[#0C2054] px-2 py-1 rounded-lg font-medium">
              {ad.differentiator}
            </span>
          )}
        </div>
      ...
      {ad.creative_url && (
        <a href={ad.creative_url} target="_blank" rel="noopener noreferrer"
          className="mt-2 flex items-center gap-1 text-[11px] text-[#0C2054] hover:text-[#1a3a7a] transition-colors">
  ...
  if (loading) {
    return (
      <div className="p-8 flex items-center justify-center min-h-screen">
        <RefreshCw className="w-8 h-8 text-[#d1d5db] animate-spin" />
      </div>
    );
  }
    ...
    <div className="p-8 space-y-6 min-h-screen">
      ...
      <div>
        <button onClick={() => router.push('/competencia')}
          className="flex items-center gap-2 text-[#9ca3af] hover:text-[#0C2054] text-sm font-medium mb-4 transition-colors group">
          <ArrowLeft className="w-4 h-4 group-hover:-translate-x-0.5 transition-transform" />
          Volver al radar
        </button>
        <div className="bg-white rounded-2xl border border-[#e8eaf0] p-6">
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 rounded-xl bg-[#0C2054] flex items-center justify-center flex-shrink-0 text-white font-bold text-lg">
              {competitor.name.charAt(0)}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-3 mb-1">
                <h1 className="text-xl font-bold text-[#0C2054]">{competitor.name}</h1>
              ...
              {competitor.location && (
                <div className="flex items-center gap-1 text-sm text-[#9ca3af] mb-2">
                  <MapPin className="w-3.5 h-3.5" />
                  {competitor.location}
                </div>
              )}
              {competitor.description && (
                <p className="text-[#6b7280] text-sm leading-relaxed mb-3">{competitor.description}</p>
              )}
              <div className="flex items-center gap-4">
                {competitor.website && (
                  <a href={competitor.website} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-1.5 text-sm text-[#0C2054] hover:text-[#1a3a7a] font-medium transition-colors">
                ...
                {competitor.practice_areas.length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {competitor.practice_areas.map(area => (
                      <span key={area} className="text-[10px] bg-[#f0f2f8] text-[#4b5563] px-2 py-0.5 rounded-full">{area}</span>
                    ))}
                  </div>
                )}
              </div>
      ...
      {/* Tabs */}
      <div className="flex gap-1 bg-white rounded-xl border border-[#e8eaf0] p-1 w-fit">
        {([
          ...
          <button key={tab.key} onClick={() => setActiveTab(tab.key)}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
              activeTab === tab.key
                ...
                : 'text-[#6b7280] hover:text-[#0C2054] hover:bg-[#f8f9fb]'
            }`}>
      ...
      {activeTab === 'digital' && (
        <div className="grid grid-cols-5 gap-5">
          ...
          <div className="col-span-3 bg-white rounded-2xl border border-[#e8eaf0] p-6">
            <h2 className="font-bold text-[#0C2054] mb-1">Perfil de Dimensiones</h2>
            <p className="text-[#9ca3af] text-xs mb-4">Puntuaciones registradas manualmente · Escala 0–10</p>
            <CompetitorRadarChart
              data={radarData}
              competitors={[{ name: competitor.name, color: '#F79C31' }]}
            />
          </div>
          ...
          <div className="col-span-2 space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-[#e8eaf0] p-5">
              <h3 className="font-bold text-[#0C2054] text-sm mb-3">Puntuaciones por dimensión</h3>
                    ...
                    <div key={key}>
                      <div className="flex items-center justify-between mb-1">
                        <div>
                          ...
                          {s?.raw_value && <span className="text-[10px] text-[#9ca3af] ml-2">{s.raw_value}</span>}
                        </div>
        ...
        <div className="grid grid-cols-5 gap-5">
          <div className="col-span-3 space-y-3">
            {competitor.ad_observations.length === 0 ? (
              <div className="bg-white rounded-2xl border border-[#e8eaf0] p-12 text-center">
                <Megaphone className="w-10 h-10 text-[#d1d5db] mx-auto mb-3" />
                <p className="text-[#9ca3af] text-sm">Sin ads registrados aún</p>
                <p className="text-[#d1d5db] text-xs mt-1">Usa el formulario para agregar anuncios observados</p>
              </div>
            ) : (
      ...
      {activeTab === 'insights' && (
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-bold text-[#0C2054]">Insights sobre {competitor.name}</h2>
          ...
          {competitor.insights.length === 0 ? (
            <div className="bg-white rounded-2xl border border-[#e8eaf0] p-12 text-center">
              <Star className="w-10 h-10 text-[#d1d5db] mx-auto mb-3" />
              <p className="text-[#9ca3af] text-sm">Sin insights registrados</p>
            </div>
          ) : (
