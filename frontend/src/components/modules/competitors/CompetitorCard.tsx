...
function ScoreBadge({ score }: { score: number | undefined }) {
  if (score === undefined) return <span className="text-[#9ca3af] text-xs">—</span>;
...
export function CompetitorCard({ competitor }: { competitor: ApiCompetitorList }) {
  ...
  return (
    <Link href={`/competencia/${competitor.id}`} className="block group">
      <div className="bg-white rounded-2xl border border-[#e8eaf0] p-5 hover:border-[#0C2054]/20 hover:shadow-md transition-all duration-200">
        {/* Header */}
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1 min-w-0 pr-2">
            ...
            <h3 className="font-semibold text-[#0C2054] text-sm leading-tight truncate group-hover:text-[#1a3a7a]">
              {competitor.name}
            </h3>
            {competitor.location && (
              <div className="flex items-center gap-1 mt-0.5">
                <MapPin className="w-3 h-3 text-[#9ca3af]" />
                <span className="text-[11px] text-[#9ca3af]">{competitor.location}</span>
              </div>
            )}
          </div>
          <ChevronRight className="w-4 h-4 text-[#d1d5db] group-hover:text-[#0C2054] flex-shrink-0 transition-colors mt-0.5" />
        </div>
        ...
        {competitor.website && (
          <div className="flex items-center gap-1.5 mb-3">
            ...
            <a
              ...
              className="text-[11px] text-[#6b7280] hover:text-[#0C2054] truncate transition-colors"
            >
              {competitor.website.replace(/^https?:\/\//, '')}
            </a>
            <ExternalLink className="w-2.5 h-2.5 text-[#d1d5db] flex-shrink-0" />
          </div>
        )}
        ...
        <div className="grid grid-cols-3 gap-1.5">
          {dimensions.map(dim => (
            <div key={dim} className="bg-[#f8f9fb] rounded-lg px-2 py-1.5 text-center">
              <div className="mb-0.5">
                <ScoreBadge score={competitor.latest_scores[dim]} />
              </div>
              <p className="text-[10px] text-[#9ca3af] font-medium leading-none">{DIMENSION_LABELS[dim]}</p>
            </div>
          ))}
        </div>
        ...
        {competitor.practice_areas.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-1">
            {competitor.practice_areas.slice(0, 3).map(area => (
              <span key={area} className="text-[10px] bg-[#f0f2f8] text-[#4b5563] px-2 py-0.5 rounded-full">
                {area}
              </span>
            ))}
            {competitor.practice_areas.length > 3 && (
              <span className="text-[10px] text-[#9ca3af]">+{competitor.practice_areas.length - 3}</span>
            )}
          </div>
        )}
      </div>
