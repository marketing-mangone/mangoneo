    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-[#f0f2f8]">
          <h2 className="font-bold text-[#0C2054] text-lg">Nuevo Insight</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                className="w-full border border-[#e5e7eb] rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0C2054]/20 bg-white"
              >
            ...
            <div>
              <label className="block text-sm font-semibold text-[#374151] mb-1.5">Impacto</label>
              <select
                value={form.impact}
          ...
          {competitors.length > 0 && !competitorId && (
            <div>
              <label className="block text-sm font-semibold text-[#374151] mb-1.5">Competidor (opcional)</label>
              <select
          ...
          <div>
            <label className="block text-sm font-semibold text-[#374151] mb-1.5">Título *</label>
            <input
              value={form.title}
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full border border-[#e5e7eb] rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0C2054]/20 resize-none"
            />
          </div>

          <div>
            <label className="block text-sm font-semibold text-[#374151] mb-1.5">Acciones recomendadas</label>
            <div className="space-y-2">
              {form.action_items.map((item, i) => (
                <div key={i} className="flex gap-2">
                  <input
                    ...
                    className="flex-1 border border-[#e5e7eb] rounded-xl px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#0C2054]/20"
                  />
                  {form.action_items.length > 1 && (
                    <button type="button" onClick={() => setForm(f => ({ ...f, action_items: f.action_items.filter((_, j) => j !== i) }))}
                      className="w-9 h-9 rounded-xl border border-[#e5e7eb] flex items-center justify-center text-[#9ca3af] hover:text-red-500 hover:border-red-200 transition-all">
              ...
              <button type="button" onClick={() => setForm(f => ({ ...f, action_items: [...f.action_items, ''] }))}
                className="flex items-center gap-1.5 text-xs text-[#0C2054] font-medium hover:text-[#1a3a7a] transition-colors">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={loading}
              className="flex-1 bg-[#0C2054] text-white rounded-xl py-2.5 text-sm font-semibold hover:bg-[#1a3a7a] disabled:opacity-50 transition-colors">
